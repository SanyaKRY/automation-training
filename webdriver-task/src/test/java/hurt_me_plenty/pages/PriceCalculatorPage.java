...
public class PriceCalculatorPage extends AbstractPage {
    ...
    @FindBy(xpath="//*[@id='compute']//md-list-item/div[@class='md-list-item-text ng-binding']")
     private List<WebElement> resultsBlockValuesEstimate;

    @FindBy(id="//*[@id='resultBlock']//*[@class='md-title']/b")
    ...
    public boolean checkValuesEstimate(String VMClass, String instanceType, String region, String localSSD, String commitmentTerm) {
    	
    	if (WaitingSomeConditions.waitForVisibilityOfElement(driver, resultsBlockValuesEstimate.get(1))
    			.getText().contains(VMClass.toLowerCase())
    			&&resultsBlockValuesEstimate.get(2).getText().contains(instanceType)
    		    &&resultsBlockValuesEstimate.get(3).getText().contains(region)
    		    &&resultsBlockValuesEstimate.get(4).getText().contains(localSSD)
    		    &&resultsBlockValuesEstimate.get(5).getText().contains(commitmentTerm)) 
    		return true;	
			return false;
    }
    
    public boolean checkTotalEstimateCost(String totalEstimateCost) {
    	if(totalEstimateCost.contains(totalEstimateCost))
    		return true;	
		    return false;
    }    	
