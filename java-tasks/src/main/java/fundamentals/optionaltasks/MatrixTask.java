	...
	for (int i = 0; i < matrix.length; i++) {
	  ...
	  for (int j = 0; j < matrix[i].length; j++) {
	    if (matrix[i][j] > 0) {
		  ...
		  System.out.println("the first positive number: " + matrix[i][indexOfFirstPositiveNumber] + " index: " +
	  ...
	  for (int j = indexOfFirstPositiveNumber+1; j < matrix.length; j++) {
		if (matrix[i][j] > 0) {
		  ...
		  System.out.println("the second positive number: " + matrix[i][indexOfSecondPositiveNumber] + " index: " + 
		      indexOfSecondPositiveNumber);
		  break;
		}
	  }
